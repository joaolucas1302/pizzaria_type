...
class PedidoController {
    static criarPedido() {
        ...
        const cliente = ClienteService_1.ClienteService.buscarClientePorId(clienteId);
        if (!cliente) {
        ...
        switch (formaPagamentoOpcao) {
            ...
            case '4':
                ...
                break;
            case '5':
                formaPagamento = Pedido_1.FormaPagamento.VALE_REFEICAO;
                break;
            default:
        ...
        try {
            const pedido = PedidoService_1.PedidoService.criarPedido({
                clienteId,
                itens,
                formaPagamento,
                observacoes: observacoes.trim() || undefined,
    ...
    static listarPedidos() {
        console.log('\n=== LISTA DE PEDIDOS ===\n');
        const pedidos = PedidoService_1.PedidoService.listarPedidos();
        if (pedidos.length === 0) {
    ...
    static atualizarStatusPedido() {
        const readline = require('readline-sync');
        console.log('\n=== ATUALIZAR STATUS DO PEDIDO ===\n');
        const id = readline.question('ID do pedido: ');
        const pedido = PedidoService_1.PedidoService.buscarPedidoPorId(id);
        if (!pedido) {
        ...
        switch (statusOpcao) {
            case '1':
                ...
                break;
            case '2':
                novoStatus = Pedido_1.StatusPedido.PREPARANDO;
                break;
            case '3':
                novoStatus = Pedido_1.StatusPedido.PRONTO;
                break;
            case '4':
        ...
        try {
            const pedidoAtualizado = PedidoService_1.PedidoService.atualizarStatusPedido(id, novoStatus);
            if (pedidoAtualizado) {
    ...
    static gerarRelatorioVendasDiario() {
        console.log('\n=== RELATÓRIO DE VENDAS DIÁRIO ===\n');
        const hoje = new Date();
        const relatorio = PedidoService_1.PedidoService.gerarRelatorioVendasDiario(hoje);
    ...
    static gerarRelatorioVendasMensal() {
        const readline = require('readline-sync');
        ...
        const relatorio = PedidoService_1.PedidoService.gerarRelatorioVendasMensal(ano, mes);
