...
class ClienteController {
    static cadastrarCliente() {
        ...
        try {
            const cliente = ClienteService_1.ClienteService.cadastrarCliente({
                nome,
                email,
                telefone,
                endereco: {
    ...
    static listarClientes() {
        console.log('\n=== LISTA DE CLIENTES ===\n');
        const clientes = ClienteService_1.ClienteService.listarClientes();
    ...
    static buscarCliente() {
        ...
        switch (opcao) {
            case '1':
                ...
                cliente = ClienteService_1.ClienteService.buscarClientePorId(id);
                break;
            case '2':
                const email = readline.question('Email do cliente: ');
                cliente = ClienteService_1.ClienteService.buscarClientePorEmail(email);
                break;
            default:
    ...
    static atualizarCliente() {
        ...
        console.log('\n=== ATUALIZAR CLIENTE ===\n');
        const id = readline.question('ID do cliente: ');
        const cliente = ClienteService_1.ClienteService.buscarClientePorId(id);
        if (!cliente) {
            console.log('\nCliente não encontrado.');
            return;
        }
        ...
        if (novoTelefone.trim())
            dadosAtualizacao.telefone = novoTelefone;
        try {
            const clienteAtualizado = ClienteService_1.ClienteService.atualizarCliente(id, dadosAtualizacao);
    ...
    static excluirCliente() {
        ...
        if (confirmacao.toLowerCase() === 's') {
            try {
                const sucesso = ClienteService_1.ClienteService.excluirCliente(id);
                if (sucesso) {
