...
exports.PromocaoService = void 0;
const Promocao_1 = require("../models/Promocao");
class PromocaoService {
    static criarPromocao(dadosPromocao) {
        // Validar dados da promoção
        this.validarDadosPromocao(dadosPromocao);
        const promocao = Promocao_1.PromocaoModel.adicionar({
            ...dadosPromocao,
            ativa: true
        });
        return promocao;
    }
    static listarPromocoes() {
        return Promocao_1.PromocaoModel.buscarTodas();
    }
    static listarPromocoesAtivas() {
        return Promocao_1.PromocaoModel.buscarAtivas();
    }
    static buscarPromocaoPorId(id) {
        const promocao = Promocao_1.PromocaoModel.buscarPorId(id);
    ...
    static aplicarMelhorPromocao(dadosPedido) {
        ...
        for (const promocao of promocoesAtivas) {
            const desconto = Promocao_1.PromocaoModel.aplicarPromocao(dadosPedido, promocao);
    ...
    static criarPromocoesPadrao() {
        const promocoes = [
            {
                nome: 'Desconto 10% - Primeira Compra',
                descricao: '10% de desconto na primeira compra',
                tipo: Promocao_1.TipoPromocao.DESCONTO_PERCENTUAL,
                valor: 10,
                valorMinimo: 30,
                dataInicio: new Date(),
            ...
            },
            {
                ...
                tipo: Promocao_1.TipoPromocao.DESCONTO_FIXO,
                valor: 5,
                valorMinimo: 50,
                dataInicio: new Date(),
                dataFim: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000), // 1 ano
    ...
    static validarDadosPromocao(dados) {
        ...
        if (dados.tipo === Promocao_1.TipoPromocao.DESCONTO_PERCENTUAL && dados.valor > 100) {
...
//# sourceMappingURL=PromocaoService.js.map